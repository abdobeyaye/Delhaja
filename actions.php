...
if (isset($_SESSION['user'])) {
    ...
    if (isset($_POST['add_order']) && $u['role'] == 'customer') {
        ...
        $delivery_fee = !empty($_POST['delivery_fee']) ? (int)$_POST['delivery_fee'] : 0;
        ...
        if (!$pickup_district_id || !$delivery_district_id) {
            setFlash('error', $t['district_required'] ?? 'Please select both pickup and delivery districts');
            header("Location: index.php");
            exit();
        }

        // Verify districts exist and are active
        $stmt = $conn->prepare("SELECT COUNT(*) FROM districts WHERE id IN (?, ?) AND is_active = 1");
        $stmt->execute([$pickup_district_id, $delivery_district_id]);
        if ($stmt->fetchColumn() != 2) {
            setFlash('error', $t['district_required'] ?? 'Please select valid districts');
            header("Location: index.php");
            exit();
        }
        ...
        // Verify delivery fee from database
        ...
        if ($delivery_fee != $verified_fee) {
            $delivery_fee = $verified_fee; // Override with verified fee
        }
    ...
    if (isset($_GET['customer_cancel']) && $u['role'] == 'customer') {
        ...
        // Allow cancellation of pending orders only
        $stmt = $conn->prepare("SELECT id, status, driver_id, points_cost FROM orders1 WHERE id=? AND (client_id=? OR customer_name=?) AND status = 'pending'");
        $stmt->execute([$order_id, $uid, $u['username']]);
        $order = $stmt->fetch();

        if ($order) {
            $conn->prepare("UPDATE orders1 SET status='cancelled', cancelled_at=NOW(), cancel_reason='Cancelled by customer before driver acceptance' WHERE id=?")
                 ->execute([$order_id]);
            setFlash('success', $t['success_order_cancelled'] ?? 'Order cancelled successfully');
        } else {
            setFlash('error', $t['err_cannot_cancel'] ?? 'Cannot cancel - order already accepted by driver');
        }
        header("Location: index.php");
        exit();
    }
